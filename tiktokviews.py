...
class AlexzxcCookies(cookiejar.CookiePolicy):
...
r = requests.Session()
r.cookies.set_policy(AlexzxcCookies())
...
def title_loop():
    ...
    if os.name == "nt":
        while True:
            os.system(f'title TikTok Views by Alexzxc ^| success: {success} fails: {fails} reqs: {reqs} rps: {rps} rpm: {rpm}')
            time.sleep(0.1)

if __name__ == "__main__":
    os.system("cls" if os.name == "nt" else "clear"); os.system("title TikTok Views by Alexzxc" if os.name == "nt" else "")
    txt = """
┏┳┓•┓ ┏┳┓  ┓   ┓┏•      
 ┃ ┓┃┏ ┃ ┏┓┃┏  ┃┃┓┏┓┓┏┏┏
 ┻ ┗┛┗ ┻ ┗┛┛┗  ┗┛┗┗ ┗┻┛┛
...
"""
    ...
    while True:
        ...
        if eval(base64.b64decode("dGhyZWFkaW5nLmFjdGl2ZV9jb3VudCgpIDwgMTAw")):
